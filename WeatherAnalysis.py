...
st.set_page_config(page_title='Weather Analysis-Amir Tabadkani', layout='wide')

st.sidebar.image(
    'https://pilbox.themuse.com/image.png?url=https:%2F%2Fassets.themuse.com%2Fuploaded%2Fcompanies%2F972%2Fsmall_logo.png%3Fv%3D9abec5f2f433df4c7adc0dadbc83efd4d7fcb5312e7bb78d5c18f6f1fc52b52d&h=630&mode=fill&prog=1&w=1200',
...
# A function to plot the hourly data
...
def get_windrose_figure_extra(st_month: int, st_day: int, st_hour: int, end_month: int, end_day: int, end_hour: int, epw, global_colorset) -> Figure:
    """Create windrose figure.
    Args:
        st_month: A number representing the start month.
        st_day: A number representing the start day.
    ...
    return lb_windrose_data.plot(title='Windrose (Dry Bulb Temperature)', show_title=True)
...
with st.container():
    ...
    st.markdown('Generate TWO windrose diagrams for the selected period and environmental parameter')
    ...
    windrose_figure_extra = get_windrose_figure_extra(windrose_st_month, windrose_st_day, windrose_st_hour, windrose_end_month,
                                          windrose_end_day, windrose_end_hour, global_epw, global_colorset)
    ...
    st.plotly_chart(windrose_figure_extra, use_container_width=True,
                    config=get_figure_config(f'Windrose_{global_epw.location.city}'))
